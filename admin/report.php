    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content mt-3">
            ...
            <?php
                ...
                $key="";
                $key = "%{$_POST['searchKey']}%";
                    $query=($key=="")?"Select * From binhluan order by kiemduyet ASC ":"Select * From binhluan Where user like ? order by kiemduyet ASC";
                $stmt=$con->prepare($query);
                    if($key!="")
                    {
                ...
                {
                    $query="Select * From binhluan order by kiemduyet ASC";
                        ...
                        <tbody>
                            ...
                            <tr>
                                <form action="" method="POST" role="form">
                                    ...
                                    <td><?php echo $row['idmaKHoc'] ?></td>
                                    <td><?php echo $row['idUsername'] ?></td>
                                    <td><?php echo $row['user'] ?></td>
                                    <td><?php echo $row['cmt'] ?></td>
                                    <td><?php echo date("H:i:s / d-m-Y",strtotime($row['date'])) ?></td>
                                    <td><input type="checkbox" name="checkKiemduyet" <?php if($row['kiemduyet']==1) echo 'checked'?> ></td>
...
if(isset($_POST['btnSubmit']))
    {
        $id=$_POST['id'];
        $kiemduyet=isset($_POST['checkKiemduyet'])? 1:0;
          $query = "UPDATE `binhluan` SET `kiemduyet` = ? WHERE id=?";
          $stmt=$con->prepare($query);
          $stmt->bind_param("ii",$kiemduyet,$id);
          $stmt->execute();
          $res=$stmt->get_result();
          echo '
                    <script type="text/javascript">
                        setTimeout('.'ReloadTrang("report.php")'.', 0);
                    </script>  ';
    }
                                ?>
                        </tbody>
